...
1. [x] Export `HOTKI_CONTROL_SOCKET` from `HotkiSession::spawn` so the UI always binds the expected bridge path.
2. [x] Add automatic reconnection/backoff in `BridgeClient` when reads return `BrokenPipe` or `ConnectionReset`.
3. [x] Cover the bridge handshake with an integration test that boots the UI runtime and ensures `BridgeRequest::Ping` succeeds before any smoketest case runs.
4. [x] Document the bridge lifecycle (init, shutdown, error handling) in `docs/testing.md` and reference it from the smoketest README.
5. [x] Restore propagation of `DriverError::NotInitialized` from `server_drive::shutdown` (or guarantee bridge handshake during session spawn) so teardown either reuses the bridge or explicitly falls back until the bridge is mandatory.

1. Stage One: Enforce Sequenced, Acknowledged Bridge Traffic (highest value)

1. [x] Attach monotonic command IDs and timestamps to every `BridgeRequest`/`BridgeResponse`.
2. [x] Buffer outstanding requests on the UI side and require explicit ACKs before processing new ones.
3. [x] Teach the smoketest driver to fail fast when sequence gaps or delayed ACKs exceed thresholds.
