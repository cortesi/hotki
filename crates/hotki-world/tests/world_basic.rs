...
fn title_update_reflected_in_snapshot() {
    ...
    rt.block_on(async move {
        ...
        while tokio::time::Instant::now() < deadline {
            let snap = world.snapshot().await;
            if let Some(w) = snap.iter().find(|w| w.id == 1) { if w.title == "New" { ok = true; break; } }
...
fn debounce_updates_within_window() {
    ...
    rt.block_on(async move {
        ...
        let world = World::spawn(mock.clone() as Arc<dyn WinOps>, WorldCfg { poll_ms_min: 10, poll_ms_max: 10, include_offscreen: false, ax_watch_frontmost: false });
        let mut rx = world.subscribe();
        ...
        while tokio::time::Instant::now() < deadline {
            ...
            if let Some(w) = s.iter().find(|w| w.id == 1) { if w.title == "T1" { ok = true; break; } }
            tokio::time::sleep(Duration::from_millis(20)).await;
        }
        ...
        while tokio::time::Instant::now() < deadline {
            let s = world.snapshot().await;
            if let Some(w) = s.iter().find(|w| w.id == 1) { if w.title == "T2" { ok2 = true; break; } }
            tokio::time::sleep(Duration::from_millis(20)).await;
        }
        ...
        while tokio::time::Instant::now() < deadline {
            let s = world.snapshot().await;
            if let Some(w) = s.iter().find(|w| w.id == 1) { if w.title == "T3" { ok3 = true; break; } }
            tokio::time::sleep(Duration::from_millis(20)).await;
        }
