...
/// Drain any immediately available world events from a cursor.
pub fn drain_events(world: &crate::WorldHandle, cursor: &mut crate::EventCursor) {
    ...
    F: FnMut(&crate::WorldEvent) -> bool,
{
    ...
    loop {
        ...
        let deadline_at = tokio::time::Instant::now() + left;
        if let Some(ev) = world.next_event_until(cursor, deadline_at).await {
