...
pub fn apply_transparent_rounded(title_match: &str, radius: f64) -> Result<()> {
    ...
    for w in windows.iter() {
        let title = w.title();
        // SAFETY: `title` is an Objective‑C NSString that lives at least for the
        // duration of the autorelease pool. `to_str` assumes UTF‑8 contents; this
        // is acceptable for window titles and falls back to lossy conversion in
        // objc2. We compare within the pool scope only.
...
pub fn set_on_all_spaces(title_match: &str) -> Result<()> {
    ...
    for w in windows.iter() {
        let title = w.title();
        // SAFETY: See note in `apply_transparent_rounded`.
        let is_match = autoreleasepool(|pool| unsafe { title.to_str(pool) == title_match });
        if is_match {
...
pub fn active_screen_frame() -> (f32, f32, f32, f32, f32) {
    ...
    // SAFETY: Reading mouse location on the AppKit main thread.
