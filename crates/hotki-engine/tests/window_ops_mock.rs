...
#[test]
fn engine_uses_window_ops_for_focus() {
    testutil::run_engine_test(async move {
...
#[test]
fn engine_fullscreen_routes_native_and_nonnative() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, _rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_raise_activates_on_match() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, _rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_place_prefers_last_raise_pid_then_clears() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, _rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_place_and_move_use_world_window_ids_for_pid_collisions() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, _rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_place_rejects_offspace_window() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        ...
        let engine = Engine::new_with_api_and_ops(api, tx, mock.clone(), false, world);
        let keys =
            keymode::Keys::from_ron("[(\"p\", \"place\", place(grid(2,2), at(0,0)))]").unwrap();
        let cfg = config::Config::from_parts(keys, config::Style::default());
        let mut engine = engine;
        engine.set_config(cfg).await.unwrap();
...
#[test]
fn engine_place_move_rejects_offspace_window() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, mut rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_raise_adopts_offspace_window() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, mut rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
        ...
        let engine = Engine::new_with_api_and_ops(api, tx, mock.clone(), false, world);
        let keys = keymode::Keys::from_ron(
            "[(\"r\", \"raise\", raise(app: \"^Target$\", title: \"Win\"))]",
        )
        .unwrap();
        let cfg = config::Config::from_parts(keys, config::Style::default());
        let mut engine = engine;
...
#[test]
fn engine_fullscreen_error_notifies() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, mut rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_raise_invalid_regex_notifies() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, mut rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
        ...
        let keys =
            keymode::Keys::from_ron("[(\"r\", \"raise\", raise(app: \"(unclosed\"))]").unwrap();
        let cfg = config::Config::from_parts(keys, config::Style::default());
        let mut engine = engine;
        engine.set_config(cfg).await.unwrap();
...
#[test]
fn engine_focus_error_propagates_notification() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        let (tx, mut rx) = mpsc::channel(16);
        let mock = Arc::new(MockWinOps::new());
...
#[test]
fn engine_place_move_uses_winops() {
    testutil::run_engine_test(async move {
        ensure_no_os_interaction();
        ...
        let world = World::spawn_view(mock.clone(), fast_world_cfg());
        let engine = Engine::new_with_api_and_ops(api, tx, mock.clone(), false, world);
        let keys = keymode::Keys::from_ron("[(\"a\", \"move left\", place_move(grid(2,2), left))]")
            .unwrap();
        let cfg = config::Config::from_parts(keys, config::Style::default());
        let mut engine = engine;
        engine.set_config(cfg).await.unwrap();
